...
def make_monopole_harm(q: float, ell: float, m: float):
    ...
    """
    # For multiple electrons it just evaluate the same function Y_qlm on each of the electrons' position
    norm_factor = np.sqrt(
        ((2 * ell + 1) / (4 * np.pi))
...
class OneRDM(PerWalkerEstimator):
    ...
    def evaluate_single_walker(
        ...
        # Get nelec sets of N-electron configurations, each with one electron replaced by r_prime.
