...
class OneAndTwoRDM(Estimator):
    ...
    def init(self, data: SolidData, rngs: PRNGKey) -> dict[str, Any]:
        ...
        cell = (
            self.scf._cell
        )  # pyscf_cell: pyscf.pbc.gto.Cell, equivalent to self._mol in Ferminet rdm.py
        ...
        self._mo_coeff = jnp.array(
            lo.orth_ao(cell, "meta-lowdin")
        ...
        if getattr(self, "phi_i", None) and getattr(self, "phi_j", None):
            ...
            assert len(indices_i) == 1, (
                f"Expected exactly 1 match for phi_i ('{self.phi_i}'), but found {len(indices_i)}: {indices_i}"
            )
            assert len(indices_j) == 1, (
                f"Expected exactly 1 match for phi_j ('{self.phi_j}'), but found {len(indices_j)}: {indices_j}"
            )
            ...
            logging.info(
                f"Sliced self._mo_coeff for specific orbitals: {self.phi_i} (idx {idx_i}) and {self.phi_j} (idx {idx_j})"
            )
        ...
        self._ao_evaluator = PBCAtomicOrbitalEvaluator.from_pyscf(
            cell
        )  # Later use with _mo_coeff to get localized meta-Lowdin orbitals
        # Use supercell folding k-points to construct Wannier-like localized orbitals
        prim_rec_vecs = get_reciprocal_vectors(self._lattice_vectors)
        self._kpts = get_supercell_kpts(self.supercell_matrix, prim_rec_vecs)

        # Scale n_up and n_down to match the supercell (which the walker samples)
        scale = len(self._kpts)
        ...
        self._aux_sampler = MCMCSampler(
            ...
            initial_width=0.5,
            adapt_frequency=10,
            sampling_proposal=make_pbc_gaussian_proposal(
                self._lattice_vectors
            ),  # By default electrons will wonder off the unit cell, but we only want to sample within the cell
        )
        ...
        return {
            ...
            "sampler_state": self._pad_sampler_state(
                sampler_state, r_prime_pool.shape[0]
            ),
    ...
    def _evaluate_mo(self, positions: jnp.ndarray) -> jnp.ndarray:
        ...
        aos_localized = jnp.mean(aos, axis=0)
        return jnp.dot(aos_localized, self._mo_coeff)
    ...
    def _pad_sampler_state(self, state_tree, size):
        ...
        return jax.tree_util.tree_map(
            lambda x: jnp.broadcast_to(x, (size,) + jnp.shape(x)), state_tree
        )

    def _unpad_sampler_state(self, state_tree):
        """Strips the dummy batch dimension back out for the sampler."""
        return jax.tree_util.tree_map(lambda x: x[0], state_tree)
    ...
    def evaluate_batch_walkers(
        ...
        batched_data: BatchedData[
            SolidData
        ],  # batched_data contains the fields: electrons, atoms, primitive_atoms, charges (electrons have shape (n_batch, n_elec, 3) )
        ...
        del (
            prev_walker_stats
        )  # Not used here, but included for compatibility with the Estimator interface.
        ...
        def do_burn(_):
            rp_out, state_out, _ = burn_in((r_prime_pool, sampler_state, burn_rng))
            return rp_out, state_out, jnp.ones_like(burn_in_counter)

        def skip(_):
            return r_prime_pool, sampler_state, burn_in_counter

        logging.info("Starting burn-in of %s steps of r_prime_pool", num_burn_in_steps)
        ...
        r_prime_per_walker_1rdm = jnp.swapaxes(
            r_prime_sweeps_1rdm, 0, 1
        )  # Shape: (batch_size, n_sweeps, 3)
        ...
        r_prime_per_walker_2rdm = jnp.transpose(
            r_prime_sweeps_2rdm, (2, 0, 1, 3)
        )  # Shape: (batch_size, n_sweeps, 2, 3)

        # =======================================================
        # 3. PURE SINGLE WALKER MATH
        # =======================================================
        def single_walker_math(walker_data, walker_rp_1rdm, walker_rp_2rdm):
            electrons = walker_data[
                self.data_field
            ]  # shape: (nelec, 3), electron positions from R
            nelec = electrons.shape[0]
            phase, log_mag = self.phase_logpsi(
                params, walker_data
            )  # phase_logpsi defined in jaqmc/src/jaqmc/app/solid/workflow.py, using function defined in src/jaqmc/app/solid/wavefunction.py
            ...
            varphi_rp_1rdm_over_f = (
                varphi_rp_1rdm / fsum_rp_1rdm[:, None]
            )  # phi(r')/f(r')

            # Calculate normalization factor (Ni_sq) based on the 1-RDM samples
            unnorm_Ni_sq = (
                jnp.abs(varphi_rp_1rdm) ** 2 / fsum_rp_1rdm[:, None]
            )  # N_i ^ 2 = E(|phi_i(r')|^2 / f(r'))
            ...
            def displace_one(a, rp):
                ...
                return self.phase_logpsi(
                    params, walker_data.merge({self.data_field: displaced})
                )  # phase(Phi(R')) and log|Phi(R')|

            vmap_displace_one = jax.vmap(
                jax.vmap(displace_one, in_axes=(None, 0)), in_axes=(0, None)
            )

            idx_up = jnp.arange(
                self.n_up
            )  # same convention as in other estimators like estimator/spin.py
            idx_down = jnp.arange(self.n_up, nelec)

            phase_prime_up, log_mag_prime_up = vmap_displace_one(
                idx_up, walker_rp_1rdm
            )  # Phi(R'), shape: (n_up, n_sweeps)
            phase_prime_down, log_mag_prime_down = vmap_displace_one(
                idx_down, walker_rp_1rdm
            )

            wf_ratio_up = (phase_prime_up / phase) * jnp.exp(
                log_mag_prime_up - log_mag
            )  # Phi(R')/Phi(R), shape: (n_up, n_sweeps)
            wf_ratio_down = (phase_prime_down / phase) * jnp.exp(
                log_mag_prime_down - log_mag
            )
            ...
            one_rdm_up = (
                jnp.einsum(  # a is for nelec, A for n_sweeps, i, j for norbs
                    "aA,ai,Aj->ij",
                    jnp.conj(wf_ratio_up),
                    jnp.conj(varphi_r[: self.n_up]),
                    varphi_rp_1rdm_over_f,
                )
                / self.n_sweeps
            )
            ...
            varphi_rp1_2rdm_over_f = (
                varphi_rp1_2rdm / fsum_rp1_2rdm[:, None]
            )  # phi(r'_a)/f(r'_a)
            varphi_rp2_2rdm_over_f = (
                varphi_rp2_2rdm / fsum_rp2_2rdm[:, None]
            )  # phi(r'_b)/f(r'_b)
            ...
            vmap_sweeps_2rdm = jax.vmap(
                displace_two, in_axes=(None, None, 0, 0)
            )  # loops over n_sweeps
            vmap_b_2rdm = jax.vmap(
                vmap_sweeps_2rdm, in_axes=(None, 0, None, None)
            )  # loops over electron b
            vmap_a_2rdm = jax.vmap(
                vmap_b_2rdm, in_axes=(0, None, None, None)
            )  # loops over electron a
            ...
            phase_prime_uu, log_mag_prime_uu = vmap_a_2rdm(
                idx_up, idx_up, rp1_2rdm, rp2_2rdm
            )  # Phi(R''_ab)
            phase_prime_dd, log_mag_prime_dd = vmap_a_2rdm(
                idx_down, idx_down, rp1_2rdm, rp2_2rdm
            )
            phase_prime_ud, log_mag_prime_ud = vmap_a_2rdm(
                idx_up, idx_down, rp1_2rdm, rp2_2rdm
            )
        ...
        walker_stats = jax.vmap(
            single_walker_math, in_axes=(batched_data.vmap_axis, 0, 0)
        )(batched_data.data, r_prime_per_walker_1rdm, r_prime_per_walker_2rdm)
