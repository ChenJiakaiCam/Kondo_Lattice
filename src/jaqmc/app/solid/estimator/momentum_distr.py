...
class MomentumDistribution(Estimator):
    ...
    supercell_matrix: Sequence[Sequence[int]] | None = runtime_dep(default=None)
    ...
    def init(self, data: SolidData, rngs: PRNGKey) -> dict[str, Any]:
        ...
        if self.supercell_matrix is not None:
            ...
            logging.info("Building PySCF supercell using ncopy: %s", ncopy)
        ...
        self._mo_coeff = jnp.array(
            lo.orth_ao(supcell, "meta-lowdin")
        ...
        key_init, _key_burn = jax.random.split(rngs)
    ...
    def _unpad_sampler_state(self, state_tree):
        ...
        return jax.tree_util.tree_map(operator.itemgetter(0), state_tree)
    ...
    def evaluate_batch_walkers(
        ...
        def single_walker_math(walker_data, walker_rp_1rdm):
            ...
            self._evaluate_mo(electrons)  # phi_i(r), shape: (nelec, norb)
            ...
            conj_ratio = jnp.conj(
                (phase_prime / phase) * jnp.exp(log_mag_prime - log_mag)
            )
            ...
            nk_total = (
                nelec
                * jnp.einsum("A,kA,A->k", conj_ratio, exp_ikr, 1.0 / fsum_rp_1rdm)
                / self.n_sweeps
            )
        ...
        walker_stats = jax.vmap(
            single_walker_math, in_axes=(batched_data.vmap_axis, 0)
        )(batched_data.data, r_prime_per_walker_1rdm)
