...
class OneAndTwoRDM_GaussianInit(Estimator):
    ...
    def init(self, data: SolidData, rngs: PRNGKey) -> dict[str, Any]:
        ...
        if self.rdm_orbitals is not None:
            indices = []
            # PySCF labels often contain trailing spaces (e.g. '0 H 1s    '). 
            # We strip all labels in the supercell once for clean matching.
            all_labels = [label.strip() for label in supcell.ao_labels()]
            
            for orb in self.rdm_orbitals:
                # Strip the target orbital name just to be safe
                target = orb.strip()
                # Use list comprehension for exact equality, bypassing partial substring matches
                matches = [i for i, label in enumerate(all_labels) if label == target]
                
                assert len(matches) == 1, f"Expected exactly 1 match for orbital '{orb}', but found {len(matches)}: {matches} (Indices: {matches})"
    ...
    def _fsum(self, positions: jnp.ndarray) -> jnp.ndarray:
        """Evaluate f(r') = sum_i |phi_i(r')|^2 using ONLY targeted RDM orbitals from rdm_orbitals"""
